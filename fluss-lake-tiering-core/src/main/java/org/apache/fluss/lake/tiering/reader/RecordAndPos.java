 ...
 * from reading a bounded (snapshot) split.
 *
 * <p>When the record is from reading log, {@code readRecordsCount} will always be {@link
 * #NO_READ_RECORDS_COUNT}.
 *
 * <p>The {@code readRecordsCount} defines the point in the snapshot reader AFTER the record. The
 * position points to where the reader should resume after this record is processed.
 *
 * <p>For example, the very first record in a snapshot split could have a {@code readRecordsCount}
 * of one.
 *
 * <p>This is the engine-agnostic counterpart of the Flink connector's {@code RecordAndPos}; it
 * wraps a {@link org.apache.fluss.client.table.scanner.batch.BatchScanner BatchScanner} drain and
 * imports nothing from any operator runtime.
 */
