...
/** End-to-end Produce test: real KafkaProducer sending records through Phase 2B Produce path. */
class KafkaProduceITCase {
    ...
    @Test
    void producerSendCompletesSuccessfully() throws Exception {
        String topic = "produce_" + System.nanoTime();
    ...
    @Test
    void producerBatchAcrossPartitionsCompletes() throws Exception {
        String topic = "batch_" + System.nanoTime();
        int partitions = 4;
    ...
    @Test
    void producerHandlesNullKey() throws Exception {
        String topic = "nullkey_" + System.nanoTime();
        ...
        try (KafkaProducer<byte[], byte[]> producer = new KafkaProducer<>(producerProps())) {
            ...
            RecordMetadata meta =
                    producer.send(new ProducerRecord<>(topic, /* partition */ 0, null, value))
                            .get();
            assertThat(meta.offset()).isGreaterThanOrEqualTo(0L);
        } finally {
            admin.deleteTopics(Collections.singletonList(topic)).all().get();
