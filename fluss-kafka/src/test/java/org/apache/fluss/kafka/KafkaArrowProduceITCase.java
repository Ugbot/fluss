...
/**
 * Verifies the Kafka produce path under the {@link LogFormat#ARROW} default introduced with the
 * {@link ConfigOptions#KAFKA_LOG_FORMAT} flag. Covers:
 *
 * <ul>
 *   <li>New log topics inherit {@code LogFormat.ARROW} on disk;
 *   <li>Single-record round-trips through the Arrow builder + format-agnostic fetch path with
 *       byte-identical key/value/headers;
 *   <li>Multi-partition fan-out and fan-in still works on Arrow batches;
 *   <li>Compacted (PK) topics keep {@code KvFormat.INDEXED} regardless of the log-format flag.
 * </ul>
 */
class KafkaArrowProduceITCase {
    ...
    @Test
    void newLogTopicIsStampedArrow() throws Exception {
        String topic = "arrow_fmt_" + System.nanoTime();
        admin.createTopics(Collections.singletonList(new NewTopic(topic, 2, (short) 1)))
                .all()
                .get();
        try {
            TableInfo info = describeTable(topic);
            assertThat(info.getTableConfig().getLogFormat()).isEqualTo(LogFormat.ARROW);
            assertThat(info.hasPrimaryKey()).isFalse();
        } finally {
    ...
    @Test
    void roundTripKeyValueHeadersThroughArrowBatch() throws Exception {
        String topic = "arrow_rt_" + System.nanoTime();
        admin.createTopics(Collections.singletonList(new NewTopic(topic, 1, (short) 1)))
                .all()
                .get();

        Random rng = new Random();
        byte[] key = bytesOf(rng, 24);
        byte[] value = bytesOf(rng, 512);
        byte[] traceId = bytesOf(rng, 8);

        try {
            try (KafkaProducer<byte[], byte[]> producer = new KafkaProducer<>(producerProps())) {
                ProducerRecord<byte[], byte[]> record =
                        new ProducerRecord<>(topic, /* partition */ 0, key, value);
                record.headers().add(new RecordHeader("traceid", traceId));
                record.headers().add(new RecordHeader("nulled", null));
                RecordMetadata meta = producer.send(record).get();
                assertThat(meta.offset()).isGreaterThanOrEqualTo(0L);
            }

            // Sanity: the table was created in ARROW format and we still got bytes back unchanged.
            assertThat(describeTable(topic).getTableConfig().getLogFormat())
            ...
            try (KafkaConsumer<byte[], byte[]> consumer =
                ...
                assertThat(round.headers().lastHeader("traceid").value()).containsExactly(traceId);
    ...
    @Test
    void compactedTopicKeepsIndexedKvFormat() throws Exception {
        String topic = "arrow_pk_" + System.nanoTime();
        ...
        try {
            TableInfo info = describeTable(topic);
            assertThat(info.hasPrimaryKey()).isTrue();
            // PK/compacted topics ignore KAFKA_LOG_FORMAT — they always use INDEXED for KV.
            assertThat(info.getTableConfig().getKvFormat()).isEqualTo(KvFormat.INDEXED);
        } finally {
            admin.deleteTopics(Collections.singletonList(topic)).all().get();
        }
    }
    ...
    @Test
    void multiPartitionBatchRoundTripsOnArrow() throws Exception {
        String topic = "arrow_multi_" + System.nanoTime();
        int partitions = 4;
        int perPartition = 8;
        ...
        try {
            try (KafkaProducer<byte[], byte[]> producer = new KafkaProducer<>(producerProps())) {
                for (int p = 0; p < partitions; p++) {
                    for (int i = 0; i < perPartition; i++) {
                        byte[] k = bytesOf(rng, 8);
                        byte[] v = ("p" + p + "-" + i).getBytes(StandardCharsets.UTF_8);
                        producer.send(new ProducerRecord<>(topic, p, k, v)).get();
                    }
                }
                producer.flush();
            }
            ...
            try (KafkaConsumer<byte[], byte[]> consumer =
                    new KafkaConsumer<>(consumerProps(topic))) {
                java.util.List<TopicPartition> tps = new java.util.ArrayList<>();
                for (int p = 0; p < partitions; p++) {
    ...
    private static Properties producerProps() {
        ...
        // Kafka 3.x silently enables idempotence when acks=all; force it off so the test only
        // exercises the Arrow append path and not the InitProducerId / producer-id allocation
        // dance (which has independent issues outside the scope of this change).
    ...
    private static Properties consumerProps(String topic) {
        ...
        props.put(
                ...
                ByteArrayDeserializer.class.getName());
        props.put(ConsumerConfig.GROUP_ID_CONFIG, "arrow-" + topic);
    ...
    private static Configuration kafkaClusterConf() {
        ...
        // Default for KAFKA_LOG_FORMAT is ARROW — assert this explicitly by leaving the option
        // unset (so any future change to the default surface here as a test failure).
        return conf;
    }
}
